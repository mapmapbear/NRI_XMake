...
public:
	...
	void update(CameraDesc &desc, float deltaTime) {
		updated = false;
		if (type == CameraType::firstperson) {
			if (moving()) {
				glm::vec3 camFront;
				...
				if (keys.forward) {
					position -= (camFront * desc.dLocal.z);
				}
				if (keys.back) {
					position -= camFront * desc.dLocal.z;
				}
				if (keys.left) {
					position += glm::normalize(glm::cross(camFront, glm::vec3(0.0f, 1.0f, 0.0f))) * desc.dLocal.x;
				}
				if (keys.right) {
					position += glm::normalize(glm::cross(camFront, glm::vec3(0.0f, 1.0f, 0.0f))) * desc.dLocal.x;
				}
