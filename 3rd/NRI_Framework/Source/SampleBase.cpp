...
void __CRTDECL operator delete(void *p) noexcept { _aligned_free(p); }
...
static void GLFW_KeyCallback(GLFWwindow *window, int32_t key, int32_t scancode,
...
static void GLFW_ButtonCallback(GLFWwindow *window, int32_t button,
                                int32_t action, int32_t mods) {
  MaybeUnused(mods);
...
static void GLFW_CursorPosCallback(GLFWwindow *window, double x, double y) {
  ...
  vec2 curPos = vec2(float(x), float(y));
...
bool SampleBase::InitUI(const nri::CoreInterface &NRI,
  ...
  // Pipeline
  {
    ...
    nri::DescriptorSetDesc descriptorSet = {
        0, descriptorRanges, helper::GetCountOf(descriptorRanges)};
    ...
    if (NRI.CreateGraphicsPipeline(device, graphicsPipelineDesc, m_Pipeline) !=
        nri::Result::SUCCESS)
      return false;
  }
  ...
  utils::LoadTextureFromMemory(format, fontWidth, fontHeight, fontPixels,
                               texture);
  ...
  NRI.GetQueue(device, nri::QueueType::GRAPHICS, 0, queue);
  {
    ...
    if (helperInterface.UploadData(*queue, &textureData, 1, nullptr, 0) !=
        nri::Result::SUCCESS)
      return false;
  }
  ...
  // Descriptor set
  {
    ...
    NRI.UpdateDescriptorRanges(*m_DescriptorSet, 0,
                               helper::GetCountOf(descriptorRangeUpdateDesc),
...
void SampleBase::BeginUI() {
  ...
  io.DisplaySize =
      ImVec2((float)m_WindowResolution.first, (float)m_WindowResolution.second);
  m_TimePrev = timeCur;

  // Update key modifiers
  io.AddKeyEvent(ImGuiMod_Ctrl,
                 IsKeyPressed(Key::LControl) || IsKeyPressed(Key::RControl));
  io.AddKeyEvent(ImGuiMod_Shift,
                 IsKeyPressed(Key::LShift) || IsKeyPressed(Key::RShift));
  io.AddKeyEvent(ImGuiMod_Alt,
                 IsKeyPressed(Key::LAlt) || IsKeyPressed(Key::RAlt));
  ...
  if ((io.ConfigFlags & ImGuiConfigFlags_NoMouseCursorChange) == 0 &&
    ...
    if (cursor == ImGuiMouseCursor_None || io.MouseDrawCursor) {
      ...
      glfwSetCursor(m_Window, m_MouseCursors[cursor]
...
void SampleBase::EndUI(const nri::StreamerInterface &streamerInterface,
  ...
  auto float2_to_unorm_16_16 = [](const vec2 &v) -> uint32_t {
    // Clamp input values to [0.0f, 1.0f] range
    float x = std::clamp(v.x, 0.0f, 1.0f);
    float y = std::clamp(v.y, 0.0f, 1.0f);

    // Convert to 16-bit unsigned normalized integers
    uint16_t ux = static_cast<uint16_t>(x * 65535.0f + 0.5f);
    uint16_t uy = static_cast<uint16_t>(y * 65535.0f + 0.5f);

    // Pack the two 16-bit values into a single 32-bit value
    return (static_cast<uint32_t>(uy) << 16) | static_cast<uint32_t>(ux);
  };
  ...
  for (int32_t n = 0; n < drawData.CmdListsCount; n++) {
    ...
    for (int32_t i = 0; i < drawList.VtxBuffer.Size; i++) {
      ...
      opt.pos[0] = v->pos.x;
      opt.pos[1] = v->pos.y;
      //   opt.uv = vec2(v->uv.x, v->uv.y);
      //   ImVec4 col = ImGui::ColorConvertU32ToFloat4(v->col);
      //   opt.col = vec4(col.x, col.y, col.z, col.w);

      opt.uv = float2_to_unorm_16_16(vec2(v->uv.x, v->uv.y));
      //   opt.col = v->col;
...
void SampleBase::RenderUI(const nri::CoreInterface &NRI,
    ...
    for (int32_t i = 0; i < drawList.CmdBuffer.Size; i++) {
      ...
      if (drawCmd.UserCallback)
        ...
        if (rect.width != 0 && rect.height != 0) {
          ...
          NRI.CmdDrawIndexed(commandBuffer, {drawCmd.ElemCount, 1, indexOffset,
                                             vertexOffset, 0});
        }
      }
...
bool SampleBase::Create(int32_t argc, char **argv, const char *windowTitle) {
  ...
  m_WindowResolution.first =
      (uint32_t)floor(m_OutputResolution.first * contentScale);
  m_WindowResolution.second =
      (uint32_t)floor(m_OutputResolution.second * contentScale);

  const GLFWvidmode *vidmode = glfwGetVideoMode(monitor);
  const uint32_t screenW = (uint32_t)vidmode->width;
  const uint32_t screenH = (uint32_t)vidmode->height;

  m_WindowResolution.first = min(m_WindowResolution.first, screenW);
  m_WindowResolution.second = min(m_WindowResolution.second, screenH);

  // Rendering output size
  m_OutputResolution.first =
      min(m_OutputResolution.first, m_WindowResolution.first);
  m_OutputResolution.second =
      min(m_OutputResolution.second, m_WindowResolution.second);

  if (m_DpiMode == 2)
    m_OutputResolution = m_WindowResolution;

  // Window creation
  bool decorated = m_WindowResolution.first != screenW &&
                   m_WindowResolution.second != screenH;
  ...
  snprintf(windowName, sizeof(windowName), "%s [%s]", windowTitle,
           cmdLine.get<std::string>("api").c_str());

  m_Window =
      glfwCreateWindow(m_WindowResolution.first, m_WindowResolution.second,
  ...
  int32_t x = (screenW - m_WindowResolution.first) >> 1;
  int32_t y = (screenH - m_WindowResolution.second) >> 1;
...
void SampleBase::RenderLoop() {
  for (uint32_t i = 0; i < m_FrameNum; i++) {
    ...
    m_MousePosPrev = vec2(float(cursorPosx), float(cursorPosy));
    m_MouseWheel = 0.0f;
  ...
  printf("FPS:\n"
         ...
         1000.0f / m_Timer.GetSmoothedFrameTime(),
         m_Timer.GetSmoothedFrameTime(),
         1000.0f / m_Timer.GetVerySmoothedFrameTime(),
         m_Timer.GetVerySmoothedFrameTime());
}
...
void SampleBase::InitCmdLineDefault(cmdline::parser &cmdLine) {
  ...
  cmdLine.add<std::string>(
      "api", 'a', "graphics API: D3D11, D3D12 or VULKAN", false, graphicsAPI,
      cmdline::oneof<std::string>("D3D11", "D3D12", "VULKAN"));
  cmdLine.add<std::string>("scene", 's', "scene", false, m_SceneFile);
  cmdLine.add<uint32_t>("width", 'w', "output resolution width", false,
                        m_OutputResolution.first);
  cmdLine.add<uint32_t>("height", 'h', "output resolution height", false,
                        m_OutputResolution.second);
  cmdLine.add<uint32_t>("frameNum", 'f', "max frames to render", false,
                        m_FrameNum);
  cmdLine.add<uint32_t>("vsyncInterval", 'v', "vsync interval", false,
                        m_VsyncInterval);
...
void SampleBase::ReadCmdLineDefault(cmdline::parser &cmdLine) {
  ...
  m_OutputResolution.first = cmdLine.get<uint32_t>("width");
  m_OutputResolution.second = cmdLine.get<uint32_t>("height");
...
void SampleBase::EnableMemoryLeakDetection(
    [[maybe_unused]] uint32_t breakOnAllocationIndex) {
