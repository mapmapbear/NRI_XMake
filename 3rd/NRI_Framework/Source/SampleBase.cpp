...
void SampleBase::GetCameraDescFromInputDevices(Camera1 &camera) {
	// cameraDesc.timeScale = 0.025f * m_Timer.GetSmoothedFrameTime();
...
bool SampleBase::Create(int32_t argc, char **argv, const char *windowTitle) {
	...
	m_WindowResolution.first = (uint32_t)floor(m_OutputResolution.first * contentScale);
	m_WindowResolution.second = (uint32_t)floor(m_OutputResolution.second * contentScale);
	...
	m_OutputResolution.first = min(m_OutputResolution.first, m_WindowResolution.first);
	m_OutputResolution.second = min(m_OutputResolution.second, m_WindowResolution.second);
	...
	// Window creation
	bool decorated = m_WindowResolution.first != screenW && m_WindowResolution.second != screenH;

	printf("Creating %swindow (%u, %u)\n", decorated ? "" : "borderless ", m_WindowResolution.first, m_WindowResolution.second);
	...
	m_Window = glfwCreateWindow(m_WindowResolution.first, m_WindowResolution.second, windowName, NULL, NULL);
	if (!m_Window) {
		glfwTerminate();
		return false;
	}
...
void SampleBase::RenderLoop() {
	...
	for (uint32_t i = 0; i < m_FrameNum; i++) {
		...
		if (HasUserInterface()) {
			ImGuiIO &io = ImGui::GetIO();
			io.DisplaySize = ImVec2((float)m_WindowResolution.first, (float)m_WindowResolution.second);
...
void SampleBase::InitCmdLineDefault(cmdline::parser &cmdLine) {
	...
	cmdLine.add<uint32_t>("width", 'w', "output resolution width", false, m_OutputResolution.first);
	cmdLine.add<uint32_t>("height", 'h', "output resolution height", false, m_OutputResolution.second);
