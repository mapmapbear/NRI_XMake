...
float4 main(InputPS input) : SV_Target
{
    ...
    float metallic = g_PushConstants.testVec.y;
    uint width = 0;
    uint height = 0;
    uint mip = 0;
    ...
    TextureCube<float4> diffuseIBL = ResourceDescriptorHeap[49];
    TextureCube<float4> specularIBL = ResourceDescriptorHeap[50];
    Texture2D<float4> BRDFTex = ResourceDescriptorHeap[51];
