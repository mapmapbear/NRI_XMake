...
void main(uint3 threadID : SV_DispatchThreadID) {
    Texture2D<float> depthTexture = ResourceDescriptorHeap[NonUniformResourceIndex(g_PushConstants.texDepth)];
    RWTexture2D<float> Hiz = ResourceDescriptorHeap[NonUniformResourceIndex(g_PushConstants.texHiZ)];
